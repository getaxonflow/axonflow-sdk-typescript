...
- **Unified Execution Tracking** (Issue #1075 - EPIC #1074): Consistent status tracking for MAP plans and WCP workflows
  ...
  - `cancelExecution(executionId, reason?)` - Cancel a unified execution (propagates to MAP `cancelPlan()` or WCP `abortWorkflow()`)
...
- **Unified execution API URLs** (EPIC #1074): `getExecutionStatus()` and `listUnifiedExecutions()` now use correct `/api/v1/unified/executions` path
