...
# package-lock.json::version (top-level AND root-package entry) is
# regenerated by `npm install` from package.json. Check it's in sync to
# catch the "forgot to run npm install after bumping package.json" case.
echo "🔒 Checking package-lock.json..."
LOCK_VER=$(python3 -c "import json; d=json.load(open('package-lock.json')); print(d.get('version',''))" || true)
if [ -z "${LOCK_VER:-}" ]; then
    echo "  ❌ package-lock.json — could not read version"
    ERRORS=$((ERRORS + 1))
elif [ "$LOCK_VER" != "$LATEST_VERSION" ]; then
    echo "  ❌ package-lock.json — version is \"$LOCK_VER\", expected \"$LATEST_VERSION\""
    ERRORS=$((ERRORS + 1))
else
