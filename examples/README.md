...
### 6. PEP Capability Handshake (`examples/pep-handshake/`)
...
`decide` sends the request as you build it. On Community, with the client id `community` or none, a `caller_identity` tenant or organization in the request is taken as given; with any other client id, and on every other edition, a `caller_identity` that names a different tenant or organization than the credentials is refused with 403 (`tenant_impersonation` or `org_impersonation`). This example sets no `caller_identity`, so it runs on Community with the credentials unset.
